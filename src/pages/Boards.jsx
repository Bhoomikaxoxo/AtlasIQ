...
import { getBoards, getPlaceById, createBoard, saveBoards } from '../services/store';
import PlaceCard from '../components/UI/PlaceCard';
import DetailModal from '../components/UI/DetailModal';
import { Folder, Plus, Trash2, ArrowLeft, Image as ImageIcon } from 'lucide-react';
import './Boards.css';
...
function Boards() {
  ...
  // Load boards on mount or updates
  ...
  useEffect(() => {
    ...
    window.addEventListener('atlas_iq_storage_update', loadBoardsData);
    return () => window.removeEventListener('atlas_iq_storage_update', loadBoardsData);
  }, []);
  ...
  // Get board cover image: uses photo of first place in board, or fallback gradient
  const getBoardCover = (board) => {
    if (board.placeIds.length > 0) {
      const place = getPlaceById(board.placeIds[0]);
      if (place) return place.photoUrl;
    }
    return null;
  };

  // Get list of place objects in a board
  const getBoardPlacesList = (board) => {
